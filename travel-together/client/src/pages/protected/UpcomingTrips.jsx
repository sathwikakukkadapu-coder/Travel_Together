...
const UpcomingTrips = () => {
  ...
  const fetchTrips = useCallback(async () => {
    ...
    try {
      ...
      setTrips(all.filter((t) => new Date(t.startDate) > now));
    ...
    <div className="container-fluid">
      <div className="tt-page-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-muted mb-0">Future trips you can join and plan with fellow travelers</p>
        </div>
        <div className="d-flex gap-2">
          <Link to="/dashboard" className="btn btn-outline-secondary">
            <i className="bi bi-arrow-left me-1" />Dashboard
          </Link>
          <Link to="/trips?tab=create" className="btn btn-primary">
      ...
      {!loading && trips.length === 0 && (
        <div className="card border-0 shadow-sm text-center py-5">
          <i className="bi bi-calendar-event fs-1 text-muted opacity-25 d-block mb-3" />
          <p className="text-muted mb-3">No upcoming trips available yet.</p>
          <Link to="/trips?tab=create" className="btn btn-primary mx-auto">
      ...
      {!loading && trips.length > 0 && (
        <div className="row g-4">
          {trips.map((trip) => {
            ...
            const openSlots     = (trip.maxMembers ?? 0) - totalOccupied;
                ...
                <div className="card border-0 shadow-sm h-100 hover-lift">
                  <div className="card-body p-4">
                    <div className="d-flex justify-content-between align-items-start mb-2">
                      ...
                      {isFull
                        ? <span className="badge bg-danger flex-shrink-0">Full</span>
                        : <span className="badge bg-primary flex-shrink-0">Upcoming</span>}
                    </div>
                    ...
                    {trip.interests?.length > 0 && (
                      <div className="d-flex flex-wrap gap-1 mb-3">
                        {trip.interests.slice(0, 3).map((i) => (
                          <span key={i} className="badge bg-light text-dark border text-capitalize" style={{ fontSize: '.65rem' }}>{i}</span>
                        ))}
                        {trip.interests.length > 3 && (
                          <span className="badge bg-light text-muted border" style={{ fontSize: '.65rem' }}>+{trip.interests.length - 3}</span>
                        )}
                      </div>
                    )}
                    ...
                    <div className="d-flex align-items-center justify-content-between mb-3">
                      <span className="small text-muted">
                        ...
                        {totalOccupied} / {trip.maxMembers ?? '∞'}
                        {!isFull && openSlots > 0 && (
                          <span className="text-success ms-1">({openSlots} open)</span>
                        )}
                      </span>
                      <button
                    ...
                    {alreadyIn ? (
                      ...
                      <button className="btn btn-danger w-100" disabled>
                        <i className="bi bi-x-circle me-2" />Slots are Full
                      </button>
                    ) : (
                      <button
